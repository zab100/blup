blup - a Bluetooth Proxy for WSA
This project aims to bring (unofficial) Bluetooth support to the Windows Subsystem for Android. For more information on WSA visit https://learn.microsoft.com/en-us/windows/android/wsa/
...
This software has not been tested extensively. This is just a proof-of-concept version. 
There may be security holes and bugs. Use this at your own risk. Assume I have modified all files and binaries.
This only works with RFCOMM devices currently. L2CAP protocols are not supported by the windows bluetooth driver.

Currently tested: Sennheiser Smart Control version 3.8.1 (Version 4.1.6 hangs, but not sure if it's a WSA or blup issue) with PXC550.
...
- If the scripts don't work try running dos2unix on all of the files
...
1) Pair the device to your computer using the Windows settings.
2) Open the settings app in the Windows Subsystem for Android:
	a) Turn Advanced Networking on if possible. There is a fallback if not, but it may not work properly.
	b) Developer > Turn Developer Mode on
	c) Connect to android using adb 
		.) This might require launching an app like files to turn the subsystem on
		..) Don't launch Android Developer Settings because this conflicts with the settings app
	d) adb shell monkey -p com.android.settings 1
3) Go to bluetooth settings, pair a new device, wait for the results to show, and pair normally
	a) There may be no indication that the device connected, but it should be marked as paired
4) Open the app that connects to the bluetooth device and the device should now show up.
