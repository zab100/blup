...
public class AvrcpNativeInterface {
    ...
    void setBrowsedPlayerResponse(int playerId, boolean success, String rootId, int numItems) {
        d("setBrowsedPlayerResponse: playerId=" + playerId
                ...
                + " rootId=" + rootId
                + " numItems=" + numItems);
                setBrowsedPlayerResponseWindows(playerId, success, rootId, numItems);
    }
    ...
    void getFolderItemsResponse(String parentId, List<ListItem> items) {
        ...
        getFolderItemsResponseWindows(parentId, items);
    }
    ...
    void sendMediaUpdate(boolean metadata, boolean playStatus, boolean queue) {
        d("sendMediaUpdate: metadata=" + metadata
                ...
                + " queue=" + queue);
        sendMediaUpdateWindows(metadata, playStatus, queue);
    }
    ...
    void sendFolderUpdate(boolean availablePlayers, boolean addressedPlayers, boolean uids) {
        d("sendFolderUpdate: availablePlayers=" + availablePlayers
                ...
                + " uids=" + uids);
        sendFolderUpdateWindows(availablePlayers, addressedPlayers, uids);
    }
    ...
    boolean connectDevice(String bdaddr) {
        ...
        return connectDeviceWindows(bdaddr);
    }

    boolean disconnectDevice(String bdaddr) {
        d("disconnectDevice: bdaddr=" + bdaddr);
        return disconnectDeviceWindows(bdaddr);
    }
    ...
    void sendVolumeChanged(String bdaddr, int volume) {
        ...
        sendVolumeChangedWindows(bdaddr, volume);
    }
    ...
    private void registerBipServerWindows(int l2capPsm){
        Log.e("MODIFYANDROIDAVRCP", "register bip");
    }
    private void unregisterBipServerWindows(){
        Log.e("MODIFYANDROIDAVRCP", "unregister bip");
    }
    private void sendMediaUpdateWindows(boolean trackChanged, boolean playState, boolean playPos){
        Log.e("MODIFYANDROIDAVRCP", "send media update");
    }
    private void sendFolderUpdateWindows(boolean availablePlayers, boolean addressedPlayers, boolean uids){
        Log.e("MODIFYANDROIDAVRCP", "send folder update");
    }
    private void setBrowsedPlayerResponseWindows(int playerId, boolean success, String rootId, int numItems){
        Log.e("MODIFYANDROIDAVRCP", "send browsed player response");
    }
    private void getFolderItemsResponseWindows(String parentId, List<ListItem> list){
        Log.e("MODIFYANDROIDAVRCP", "send folder items response");
    }

    private boolean connectDeviceWindows(String bdaddr){
        Log.e("MODIFYANDROIDAVRCP", "connect device " + bdaddr);
        return false;
    }
    private boolean disconnectDeviceWindows(String bdaddr){
        Log.e("MODIFYANDROIDAVRCP", "disconnect device " + bdaddr);
        return false;
    }
    private void sendVolumeChangedWindows(String bdaddr, int volume){
        Log.e("MODIFYANDROIDAVRCP", "send volume changed " + bdaddr);
    }
    private void setBipClientStatusWindows(String bdaddr, boolean connected){
        Log.e("MODIFYANDROIDAVRCP", "send bip client status " + bdaddr);
    }
