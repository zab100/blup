...
public class SdpManager {
    ...
    private void startSearch() {
        ...
        if ((inst != null) && (!sSearchInProgress)) {
            ...
            sdpSearchWindows(inst.getDevice().getAddress(), inst.getUuid().toString());
    ...
    private int sdpCreateMapMnsRecordWindows(String serviceName, int rfcommChannel, int l2capPsm, int version, int features){
        return -1;
    }

    private int sdpCreatePbapPceRecordWindows(String serviceName, int version){
        return -1;
    }

    private int sdpCreatePbapPseRecordWindows(String serviceName, int rfcommChannel, int l2capPsm, int version, int repositories, int features){
        return -1;
    }

    private int sdpCreateOppOpsRecordWindows(String serviceName, int rfcommChannel, int l2capPsm, int version, byte[] formatsList){
        return -1;
    }

    private int sdpCreateSapsRecordWindows(String serviceName, int rfcommChannel, int version){
        return -1;
    }

    private boolean sdpRemoveSdpRecordWindows(int recordId){
        return false;
    }
}
