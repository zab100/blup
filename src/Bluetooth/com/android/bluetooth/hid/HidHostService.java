...
public class HidHostService extends ProfileService {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MESSAGE_CONNECT: {
                    ...
                    if (!connectHidWindows(Utils.getByteAddress(device))) {
                ...
                case MESSAGE_DISCONNECT: {
                    ...
                    Attributable.setAttributionSource(device,
                            ActivityThread.currentAttributionSource());
                    if (!disconnectHidWindows(Utils.getByteAddress(device))) {
                        broadcastConnectionState(device, BluetoothProfile.STATE_DISCONNECTING);
                        broadcastConnectionState(device, BluetoothProfile.STATE_DISCONNECTED);
                        break;
                    }
                ...
                case MESSAGE_CONNECT_STATE_CHANGED: {
                    ...
                    if (halState == CONN_STATE_CONNECTED
                        ...
                        virtualUnPlugWindows(Utils.getByteAddress(device));
                    } else {
                ...
                case MESSAGE_GET_PROTOCOL_MODE: {
                    ...
                    Attributable.setAttributionSource(device,
                            ActivityThread.currentAttributionSource());
                    if (!getProtocolModeWindows(Utils.getByteAddress(device))) {
                ...
                case MESSAGE_VIRTUAL_UNPLUG: {
                    ...
                    Attributable.setAttributionSource(device,
                            ActivityThread.currentAttributionSource());
                    if (!virtualUnPlugWindows(Utils.getByteAddress(device))) {
                ...
                case MESSAGE_SET_PROTOCOL_MODE: {
                    ...
                    if (!setProtocolModeWindows(Utils.getByteAddress(device), protocolMode)) {
                ...
                case MESSAGE_GET_REPORT: {
                    ...
                    if (!getReportWindows(Utils.getByteAddress(device), reportType, reportId,
                            bufferSize)) {
                ...
                case MESSAGE_SET_REPORT: {
                    ...
                    if (!setReportWindows(Utils.getByteAddress(device), reportType, report)) {
                ...
                case MESSAGE_GET_IDLE_TIME: {
                    ...
                    Attributable.setAttributionSource(device,
                            ActivityThread.currentAttributionSource());
                    if (!getIdleTimeWindows(Utils.getByteAddress(device))) {
                ...
                case MESSAGE_SET_IDLE_TIME: {
                    ...
                    if (!setIdleTimeWindows(Utils.getByteAddress(device), idleTime)) {
                        Log.e(TAG, "Error: get idle time native returns false");
                    }
                }
                break;
    ...
    boolean sendData(BluetoothDevice device, String report) {
        ...
        return sendDataWindows(Utils.getByteAddress(device), report);
    }
    ...
    public static final byte HANDSHAKE = 0x0;
    public static final byte GET_REPORT = 0x4;
    public static final byte SET_REPORT = 0x5;
    public static final byte GET_PROTOCOL = 0x6;
    public static final byte SET_PROTOCOL = 0x7;
    public static final byte DATA = 0xA;

    // private final HashMap<byte[], Socket> connectedSockets = new HashMap<>();

    private void decodeHeader(byte[] address, byte[] payload, Object... extra){
        byte hidpMessageType = (byte) (payload[0] >> 4);
        byte parameter = (byte) (payload[0] & 0b00001111);

        switch(hidpMessageType){
            case HANDSHAKE:
                onHandshake(address, parameter);
                break;
            case DATA:
                onGetReport(address, payload, (int)extra[0]);
                break;
            default:
                Log.e("MODIFYANDROIDCONNECTHID", "host should never receive a hidp message that is not HANDSHAKE or DATA");
                break;
        }
    }

    private boolean connectHidWindows(byte[] btAddress){
        Log.e("MODIFYANDROIDHID", "connect hid " + Utils.byteArrayToString(btAddress));
        WindowsCommunicator.sendToWindows(new BluetoothDeviceMessage(NetworkingRequest.REQUEST_BLUETOOTH_HID_CONNECTION, Utils.byteArrayToString(btAddress)));
        return true;
    }

    private boolean disconnectHidWindows(byte[] btAddress){
        Log.e("MODIFYANDROIDHID", "disconnect hid " + Utils.byteArrayToString(btAddress));

        return true;
    }

    //TODO: synchronize all writing methods and auto create a thread for them
    //probably make a new class that wraps socket
    private boolean getProtocolModeWindows(byte[] btAddress){
        Log.e("MODIFYANDROIDHID", "get protocol mode hid " + Utils.byteArrayToString(btAddress));
        // new Thread(() -> {
        //     connectedSockets.get(btAddress).getOutputStream();
        // }).start();

        return false;
    }

    private boolean virtualUnPlugWindows(byte[] btAddress){
        Log.e("MODIFYANDROIDHID", "virtual unplug hid " + Utils.byteArrayToString(btAddress));
        return false;
    }

    private boolean setProtocolModeWindows(byte[] btAddress, byte protocolMode){
        Log.e("MODIFYANDROIDHID", "set protocol mode hid " + Utils.byteArrayToString(btAddress));
        return false;
    }

    private boolean getReportWindows(byte[] btAddress, byte reportType, byte reportId, int bufferSize){
        String address = Utils.byteArrayToString(btAddress);
        Log.e("MODIFYANDROIDHID", "get report hid " + address + " report type " + reportType + " reportid " + reportId + " buffersize " + bufferSize);

        byte[] payload = new byte[4];
        payload[0] = (byte) ((GET_REPORT << 4) | (0b1 << 3) | (reportType));
        payload[1] = reportId;
        payload[2] = (byte) (bufferSize & 0b0000000011111111);
        payload[3] = (byte) ((bufferSize & 0b1111111100000000) >> 8);

        WindowsCommunicator.sendToWindows(new BluetoothDeviceMessage(NetworkingRequest.REQUEST_BLUETOOTH_L2CAP_SEND, address, payload), null, (success) -> {
            if(success){
                WindowsCommunicator.sendToWindows(new BluetoothDeviceMessage(NetworkingRequest.REQUEST_BLUETOOTH_L2CAP_READ, address), (response) -> {
                    decodeHeader(btAddress, response.getL2capBytes(), bufferSize);
                });
            }
        });
        return true;
    ...
    private boolean setReportWindows(byte[] btAddress, byte reportType, String report){
        String address = Utils.byteArrayToString(btAddress);
        Log.e("MODIFYANDROIDHID", "set report hid " + address);

        byte header = (byte) ((SET_REPORT << 4) | reportType);
        byte[] reportBytes = report.getBytes();
        byte[] payload = new byte[reportBytes.length + 1];
        payload[0] = header;
        for(int i = 0; i < reportBytes.length; i++){
            payload[i + 1] = reportBytes[i];
        }

        WindowsCommunicator.sendToWindows(new BluetoothDeviceMessage(NetworkingRequest.REQUEST_BLUETOOTH_L2CAP_SEND, address, payload));
        return true;
    }

    private boolean sendDataWindows(byte[] btAddress, String report){
        String address = Utils.byteArrayToString(btAddress);
        Log.e("MODIFYANDROIDHID", "send data hid " + address);
    ...
    private boolean setIdleTimeWindows(byte[] btAddress, byte idleTime){
        Log.e("MODIFYANDROIDHID", "set idle time windows hid " + Utils.byteArrayToString(btAddress));
        return false;
    }

    private boolean getIdleTimeWindows(byte[] btAddress){
        Log.e("MODIFYANDROIDHID", "get idle time windows hid " + Utils.byteArrayToString(btAddress));
        return false;
    }
}
