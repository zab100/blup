...
import android.bluetooth.zbt.ForwarderReceiver;
...
import com.zbt.networking.BluetoothDeviceMessage;
...
public final class BluetoothAdapter {
    ...
    BluetoothAdapter(IBluetoothManager managerService, AttributionSource attributionSource) {
        // Log.e("SERVICEMODIFYANDROIDSYSTEM", "registering ");
        // ServiceManager.addService("bluetoothforwarder", new ForwarderReceiver());
        // Log.e("SERVICEMODIFYANDROIDSYSTEM", "registeded ");
        ...
        // Log.e("SERVICEMODIFYANDROIDSYSTEM", "begin ");
        // IBinder binder = ServiceManager.getService("bluetoothforwarder");
        // ForwarderReceiver receiver = (ForwarderReceiver) binder;
        // Log.e("SERVICEMODIFYANDROIDSYSTEM", "ran " + receiver);
    }

    private BluetoothResponse sendToWindows(BluetoothDeviceMessage message){
        BluetoothResponse[] response = new BluetoothResponse[1];
        Thread t = new Thread(){
            @Override
            public void run(){
                try {
                    ...
                    IWifiManager manager = IWifiManager.Stub.asInterface(binder);
                    DhcpInfo dhcp = manager.getDhcpInfo("android");
                    String address = Formatter.formatIpAddress(dhcp.gateway);
                    Socket clientSocket = new Socket(address, 5057);

                    Log.e("MODIFYORWARDER", "connecting to " + address);
                    ...
                    try {
                        ...
                        response[0] = (BluetoothResponse) inFromWindows.readObject();
        ...
        t.start();
        try {
            Log.e("MODIFYANDROIDSYSTEM", "joining thread");
            t.join();
    ...
    public Set<BluetoothDevice> getBondedDevices() {
        ...
        try {
            mServiceLock.readLock().lock();
            if (mService != null) {
                Log.e("MODIFYANDROIDSYSTEM", "bonded devices are " + toDeviceSet(Attributable.setAttributionSource(
                    Arrays.asList(mService.getBondedDevices(mAttributionSource)),
                    mAttributionSource)).toString());
        ...
        // Log.e("MODIFYANDROIDSYSTEM", "Requesting Bonded Devices");

        // BluetoothDeviceMessage message = new BluetoothDeviceMessage();
        // message.setRequestPairedDevices(true);
        // Log.e("MODIFYANDROIDSYSTEM", "created message and sending to windows");
        // BluetoothResponse response = sendToWindows(message);
        // Log.e("MODIFYANDROIDSYSTEM", "received response from message handler");
        // Map<String, String> deviceAddressesAndNames = response.getPairedDevices();
        // if(deviceAddressesAndNames == null){
        //     return null;
        // }
        // Set<BluetoothDevice> pairedDevices = new HashSet<>();
        // deviceAddressesAndNames.forEach((address, name) -> {
        //     pairedDevices.add(new BluetoothDevice(address, name));
        // });
        // return pairedDevices;
    }

    public static String getDefaultForwarderAddress() throws RemoteException{
        IBinder binder = ServiceManager.getService(Context.WIFI_SERVICE);
        IWifiManager manager = IWifiManager.Stub.asInterface(binder);
        DhcpInfo dhcp = manager.getDhcpInfo("android");
        String address = Formatter.formatIpAddress(dhcp.gateway);
